...
// Every admin needs a referral alias so partners can sign up under them
// directly (yoursite.com/?ref=alias). Backfills any admin created before
// this feature existed, or created without one for any reason.
function backfillAdminSlugs() {
  const rows = db.prepare(`SELECT id, first_name FROM users WHERE role='admin' AND (ref_slug IS NULL OR ref_slug = '')`).all();
  for (const row of rows) {
    const slug = generateUniqueSlug(row.first_name || 'manager', row.id);
    db.prepare('UPDATE users SET ref_slug=? WHERE id=?').run(slug, row.id);
  }
}
backfillAdminSlugs();
