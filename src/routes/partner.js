...
router.get('/settings', (req, res) => {
  ...
  if (u && u.owner_admin_id) {
    const owner = db.prepare(`SELECT payout_method_url, payout_urls FROM users WHERE id=? AND role='admin'`).get(u.owner_admin_id);
    if (owner) {
      const allowed = [...JSON.parse(owner.payout_urls || '[]'), owner.payout_method_url];
      settings.payout_method_url = u.payout_method_url && allowed.includes(u.payout_method_url)
        ? u.payout_method_url : (owner.payout_method_url || settings.payout_method_url);
    }
  }
