...
router.get('/partners/:id', (req, res) => {
  ...
  const owner = db.prepare('SELECT * FROM users WHERE id=?').get(u.owner_admin_id);
  const payoutUrls = owner ? JSON.parse(owner.payout_urls || '[]') : [];
  if(owner && owner.payout_method_url && !payoutUrls.includes(owner.payout_method_url)) payoutUrls.unshift(owner.payout_method_url);
  res.json({ ...serializePartner(u), activity, transactions, payoutUrls, payoutUrl: u.payout_method_url || '' });
});
...
function serializeAdmin(u) {
  return {
    ...
    refSlug: u.ref_slug || '',
    payoutMethodUrl: u.payout_method_url || '',
    payoutUrls: JSON.parse(u.payout_urls || '[]'),
    hasTelegramBot: !!(u.telegram_bot_token && u.telegram_chat_id),
    telegramBotTokenMasked: u.telegram_bot_token ? '••••' + String(u.telegram_bot_token).slice(-6) : '',
...
// Lets the super admin give a specific manager their own "set up payout
// method" link. Their partners will see this instead of the site-wide
// default (src/routes/partner.js resolves it per-partner by owner_admin_id).
router.put('/admins/:id/payout-url', requireSuperAdmin, (req, res) => {
  const { urls } = req.body || {};
  if (!Array.isArray(urls) || urls.length > 20 || urls.some(url => {
    try { const u = new URL(url); return u.protocol !== 'https:' || !!u.username || !!u.password; } catch { return true; }
...
// Gives a manager their own Telegram bot for notifications about their own
// partners' actions (registrations, logins, payouts, messages...) instead of
// the site-wide bot. Pass empty strings to clear it and fall back to the
// site-wide TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID again. Super admin only,
// since this exposes/replaces a bot token.
router.put('/admins/:id/telegram', requireSuperAdmin, (req, res) => {
  const { botToken, chatId } = req.body || {};
  const target = db.prepare(`SELECT * FROM users WHERE id=? AND role='admin'`).get(req.params.id);
  if (!target) return res.status(404).json({ error: 'Admin not found.' });
  db.prepare('UPDATE users SET telegram_bot_token=?, telegram_chat_id=? WHERE id=?')
    .run(botToken ? String(botToken).trim() : '', chatId ? String(chatId).trim() : '', target.id);
  res.json(serializeAdmin(db.prepare('SELECT * FROM users WHERE id=?').get(target.id)));
});
...
function serializeProfitEntry(e) {
  const split = splitAmount(e.amount, e.expense_amount);
  return {
    id: e.id, adminId: e.admin_id, date: e.entry_date, partnerTg: e.partner_tg,
    amount: e.amount, expenseAmount: e.expense_amount || 0, createdAt: e.created_at,
    managerShare: split.managerShare, gatewayCost: split.gatewayCost,
    ownerGross: split.ownerGross, ownerNet: split.ownerNet
  };
}
...
router.put('/report/activity', (req, res) => {
  ...
  const tg = Math.max(0, Number(tgPosts) || 0);
  const groups = Math.max(0, Number(groupsCreated) || 0);
  db.prepare(`
...
router.get('/report/profit', (req, res) => {
  ...
  res.json({
    entries: entries.map(serializeProfitEntry),
...
router.post('/report/profit', (req, res) => {
  ...
  const exp = Number.isFinite(Number(expenseAmount)) ? Math.max(0, Number(expenseAmount)) : 0;
  const d = isValidDateStr(date) ? date : todayStr();
  ...
  res.json(serializeProfitEntry(db.prepare('SELECT * FROM admin_profit_entries WHERE id=?').get(info.lastInsertRowid)));
});
...
router.put('/report/profit/:id', (req, res) => {
  ...
  const exp = expenseAmount === undefined ? entry.expense_amount : Math.max(0, Number(expenseAmount) || 0);
  db.prepare('UPDATE admin_profit_entries SET partner_tg=?, amount=?, expense_amount=? WHERE id=?').run(tg, amt, exp, entry.id);
  res.json(serializeProfitEntry(db.prepare('SELECT * FROM admin_profit_entries WHERE id=?').get(entry.id)));
});
...
function serializeExpense(e) {
  return { id: e.id, date: e.expense_date, label: e.label || '', amount: e.amount, note: e.note || '', createdAt: e.created_at };
}
...
router.post('/finance/expenses', requireSuperAdmin, (req, res) => {
  ...
  if (!Number.isFinite(amt) || amt <= 0) return res.status(400).json({ error: 'Enter an amount greater than zero.' });
  const d = isValidDateStr(date) ? date : todayStr();
  const info = db.prepare('INSERT INTO company_expenses (expense_date, label, amount, note, created_by) VALUES (?,?,?,?,?)')
    .run(d, label ? String(label).trim() : '', amt, note ? String(note).trim() : '', req.userId);
...
router.put('/finance/expenses/:id', requireSuperAdmin, (req, res) => {
  ...
  if (!Number.isFinite(amt) || amt < 0) return res.status(400).json({ error: 'Invalid amount.' });
  db.prepare('UPDATE company_expenses SET expense_date=?, label=?, amount=?, note=? WHERE id=?').run(
...
router.get('/finance/entries', requireSuperAdmin, (req, res) => {
  ...
  const rows = date
    ...
    : db.prepare(`
      ...
      `).all();
  res.json(rows.map(r => Object.assign(serializeProfitEntry(r), {
...
router.get('/stats/daily', requireSuperAdmin, (req, res) => {
  const date = isValidDateStr(req.query.date) ? req.query.date : todayStr();
  const admins = db.prepare(`SELECT * FROM users WHERE role='admin' ORDER BY is_super_admin DESC, id ASC`).all();
  const rows = admins.map(a => {
    const registrations = db.prepare(`
      SELECT COUNT(*) AS c FROM users WHERE role='partner' AND owner_admin_id=? AND date(created_at)=?
    `).get(a.id, date).c;
    const partnersCredited = db.prepare(`
      SELECT COUNT(DISTINCT t.user_id) AS c FROM transactions t JOIN users u ON u.id = t.user_id
      WHERE t.type='credit' AND u.owner_admin_id=? AND date(t.created_at)=?
    `).get(a.id, date).c;
    ...
    return {
      ...
      dayProfitEntries: dayEntries.map(serializeProfitEntry),
      dayProfitTotal: dayEntries.reduce((s, e) => s + e.amount, 0),
      totalEarning: totalAll / 2
    };
  });
  res.json({ date, admins: rows });
});
...
router.put('/partners/:id/payout-url', (req, res) => {
  ...
  if (!me.is_super_admin && p.owner_admin_id !== me.id) return res.status(403).json({error:'Access denied.'});
  const owner = db.prepare('SELECT * FROM users WHERE id=?').get(p.owner_admin_id);
  const allowed = owner ? [...JSON.parse(owner.payout_urls || '[]'), owner.payout_method_url] : [];
