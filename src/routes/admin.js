...
function serializeProfitEntry(e) {
  return { id: e.id, adminId: e.admin_id, date: e.entry_date, partnerTg: e.partner_tg, amount: e.amount, createdAt: e.created_at };
}
...
router.post('/report/profit', (req, res) => {
  const { partnerTg, amount, date } = req.body || {};
  ...
  const d = isValidDateStr(date) ? date : todayStr();
  const info = db.prepare('INSERT INTO admin_profit_entries (admin_id, entry_date, partner_tg, amount) VALUES (?,?,?,?)')
    .run(req.userId, d, String(partnerTg).trim(), amt);
...
router.put('/report/profit/:id', (req, res) => {
  ...
  const { partnerTg, amount } = req.body || {};
  ...
  db.prepare('UPDATE admin_profit_entries SET partner_tg=?, amount=? WHERE id=?').run(tg, amt, entry.id);
