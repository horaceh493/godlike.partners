...
// Each manager's own self-reported daily activity: how many messages they
// wrote in Telegram that day, and how many groups they created. One row per
// (admin, date) -- upserted from the admin's "My report" tab.
db.exec(`
CREATE TABLE IF NOT EXISTS admin_daily_activity (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
...
// A running ledger a manager fills in themselves: a partner's Telegram handle
// plus how much money came out of them. The sum of every entry, split in
// half, is that manager's earning. The super admin can correct any entry's
// amount (e.g. if a manager mistyped it).
db.exec(`
...
// Optional per-entry cost that reduces the OWNER's cut of that specific
// profit record (e.g. a proxy/SIM cost tied to that one partner). Defaults
// to 0 -- i.e. nothing is deducted unless someone fills it in.
try { db.exec('ALTER TABLE admin_profit_entries ADD COLUMN expense_amount REAL NOT NULL DEFAULT 0'); } catch (e) {}

// Company-wide costs that are NOT tied to any specific partner (electricity,
// hosting, proxies, etc.) -- shown separately from the per-entry expense
// above, and subtracted from the owner's net earning on the finance panel.
db.exec(`
...
try { db.exec("ALTER TABLE users ADD COLUMN payout_urls TEXT DEFAULT '[]'"); } catch (e) {}
