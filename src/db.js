...
CREATE TABLE IF NOT EXISTS users (
  ...
  ref_code TEXT UNIQUE,                               -- admin's own referral code (admins only)
  referred_by INTEGER REFERENCES users(id),           -- which admin this partner signed up under (partners only)
  is_super INTEGER NOT NULL DEFAULT 0,                -- super-admins see every partner, not just their own referrals
...
try { db.exec('ALTER TABLE users ADD COLUMN referred_by INTEGER REFERENCES users(id)'); } catch (e) {}
try { db.exec('ALTER TABLE users ADD COLUMN is_super INTEGER NOT NULL DEFAULT 0'); } catch (e) {}
try { db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_users_ref_code ON users(ref_code)'); } catch (e) {}
